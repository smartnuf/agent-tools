...
| Upgrade from the previous supported release is tested | complete | Issue #55's cross-platform lifecycle test checksum-verifies the published v0.1.1 bundle, installs its exact wheel directly like the documented prerelease command, replaces that direct receipt with the shared current CI wheel, and verifies the current-only CLI surface |
