...
class ReleaseTests(unittest.TestCase):
    ...
    def test_pypi_smoke_workflow_uses_the_public_index_on_all_platforms(self) -> None:
        ...
        self.assertIn('$package = "smartnuf-agent-tools"', workflow)
        self.assertIn('$pin = "$package==$env:RELEASE_VERSION"', workflow)
        self.assertIn('package="smartnuf-agent-tools"', workflow)
        self.assertIn('pin="smartnuf-agent-tools==$RELEASE_VERSION"', workflow)
    ...
    def test_current_tag_matches_package_version(self) -> None:
        self.assertEqual(release.verify_tag("v0.2.1"), "v0.2.1")
        self.assertEqual(release.package_version(), "0.2.1")
