...
AGENT_TOOLS_TEST_LOG="$LOG" PATH="$TEST_BIN:/usr/bin:/bin" sh "$ROOT/scripts/install-native.sh"
[ "$(sed -n '1p' "$LOG")" = 'update' ]
...
echo 1000
EOF
if AGENT_TOOLS_TEST_LOG="$LOG" PATH="$TEST_BIN:/usr/bin:/bin" sh "$ROOT/scripts/install-native.sh" 2>/dev/null; then
