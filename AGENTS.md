...
- At project starts and material problem-envelope expansions, follow the proportional [`Intent → Characterise → Discover → Map → Plan → Specify → Execute → Review → Learn`](docs/development-workflow.md#top-level-engineering-cycle) cycle. Map architecture and workflow from the problem, risk, and knowledge profile; ordinary low-risk work stays lightweight.
- Resolve material knowledge gaps with targeted primary-source research, prior art, experiments, or real-environment evidence before implementation when the result can affect design, risk, specification, or workflow.
